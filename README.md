...
1. Explain what a token is used for.
   ...
   A token is used to validate whether a user is authorized to view certain content. It is received from an API call when the proper credentials are posted to the server.

2. What steps can you take in your web apps to keep your data secure?

   Making sure that the proper tokens are given out to the right users helps to keep information guarded against those who are not supposed to see it. You can also set up Private Routes so that pages will not be visible unless a token is held by the current user.

3. Describe how web servers work.

   Web servers are just computers that act as a place to store information about a web application. When a client requests access to information on a server, the call to the server can be checked with JWT's to make sure the user is authorized to view certain data. The data is then passed back to the client to use.
